...
RegisterServerEvent('esx_advanced_holdup:tooFar')
AddEventHandler('esx_advanced_holdup:tooFar', function(currentStore)
	...
	for i=1, #xPlayers, 1 do
		...
		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('esx_advanced_holdup:killBlip', xPlayers[i])
		end
	end

	if robbers[_source] then
		TriggerClientEvent('esx_advanced_holdup:tooFar', _source)
		robbers[_source] = nil
		TriggerClientEvent('esx:showNotification', _source, _U('robbery_cancelled_at', Stores[currentStore].nameOfStore))
	end
end)

RegisterServerEvent('esx_advanced_holdup:robberyStarted')
AddEventHandler('esx_advanced_holdup:robberyStarted', function(currentStore)
	local _source  = source
	...
	if Stores[currentStore] then
		...
		if not rob then
			if cops >= Config.PoliceNumberRequired then
				rob = true
                --start alarm config from here
				for i=1, #xPlayers, 1 do
					local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
					if xPlayer.job.name == 'police' then
						TriggerClientEvent('esx:showNotification', xPlayers[i], _U('rob_in_prog', store.nameOfStore))
						TriggerClientEvent('esx_advanced_holdup:setblip', xPlayers[i], Stores[currentStore].position)
					end
				end
				...
				for i=1, #xPlayers, 1 do
					...
					if xPlayer.job.name == 'ambulance' then
						--TriggerClientEvent('esx:showNotification', xPlayers[i], _U('rob_in_prog', store.nameOfStore)) no need to be in this part only if you want ems to see this, if not remove this
						TriggerClientEvent('esx_advanced_holdup:setblip', xPlayers[i], Stores[currentStore].position)
					end
				end
				...
				end --add some more jobs as your jobs data.
				...
				TriggerClientEvent('esx_advanced_holdup:currentlyRobbing', _source, currentStore)
				TriggerClientEvent('esx_advanced_holdup:startTimer', _source)
				...
				SetTimeout(store.secondsRemaining * 1000, function()
					if robbers[_source] then
						rob = false
						if xPlayer then
							TriggerClientEvent('esx_advanced_holdup:robberyComplete', _source, store.reward)
